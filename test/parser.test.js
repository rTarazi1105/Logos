...
const syntaxChecks = [
  ...
  ["even longer if", "mod m() { if 0 > 1 then print(1) else if false then {print(1); return 1 } else print(0)"],
  ...
  ["returning up", "mod m<A: SomeClass, B>(a: A, b: B) -> B { sum = 0; last = for i in [0, 1, 2] do { sum++; if i == 1 then return.0 i}; if sum != 3 then return b else { if last == 1 return.. b.inverse(); }; }"],
  ["chained ors", "mod m(a: bool, b: bool, c: bool) { if a and b and c then print(\"ok\"); }"],
...
const syntaxErrors = [
  ...
  ["starting with a number in a mod", "mod m() { 1abc = 2; }", /Line 1, col 11:/],
]
// LINE 101 = 64 + 37
describe("The parser", () => {
